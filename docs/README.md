# Документация ASIC Monitor

## Пользователям

- [Установка, сети, доступ к ASIC, отчёты и устранение проблем](user_guide.md)
- [Android: установка APK, возможности и локальная сборка](../apps/android/README.md)
- [Что нового](../CHANGELOG.md)
- [Диагностика и аппаратные испытания](testing/README.md)
- [Матрица проверки моделей и прошивок — Excel](testing/asic_acceptance.xlsx)
- [Приёмка Android на телефоне и оборудовании](testing/android_acceptance.md)

## Разработчикам

- [Участие в разработке](../CONTRIBUTING.md)
- [Архитектура и границы текущего рефакторинга](architecture/scanner_refactoring.md)
- [Добавление устройств и прошивок](development/adding_device.md)
- [Сборка EXE и выпуск релиза](development/windows_release.md)
- [Техническое задание](technical_specification.md)
- [План развития](../ROADMAP.md)

## Справочные материалы

- [Каталог профилей](10_device_profiles.md)
- [Матрица команд](09_control_commands_matrix.md)
- [Правила проверки управления](11_control_verification.md)
- API: [Bitmain](02_antminer_stock_api.md), [Whatsminer](03_whatsminer_api.md), [VNish](04_vnish_api.md), [Elphapex](01_elphapex_api.md), [iPollo](05_ipollo_api.md), [Jasminer](06_jasminer_api.md), [Avalon](07_avalon_api.md), [CGMiner](08_cgminer_web_api.md).

Справочная документация описывает протоколы и служит основанием для реализации. Она не заменяет испытания конкретного устройства и прошивки.
